// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using NpgsqlTypes;
using amorphie.user.data;

#nullable disable

namespace amorphie.user.data.Migrations
{
    [DbContext(typeof(UserDBContext))]
    [Migration("20230329092427_TsVectorTables")]
    partial class TsVectorTables
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "7.0.0")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("SecurityImage", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CreatedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<string>("Image")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ModifiedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ModifiedByBehalfOf")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.ToTable("SecurityImages");

                    b.HasData(
                        new
                        {
                            Id = new Guid("6b8e63e7-531c-4fa6-940f-3744545ba994"),
                            CreatedAt = new DateTime(2023, 3, 29, 9, 24, 26, 910, DateTimeKind.Utc).AddTicks(2245),
                            CreatedBy = new Guid("1a6ab381-200b-494f-ba21-aad861649f7a"),
                            CreatedByBehalfOf = new Guid("9a11ff22-649c-47d4-a1f1-d1b689e50188"),
                            Image = "",
                            ModifiedAt = new DateTime(2023, 3, 29, 9, 24, 26, 910, DateTimeKind.Utc).AddTicks(2246),
                            ModifiedBy = new Guid("f684d298-40a8-4e1a-8573-3fdd8c43fc09"),
                            ModifiedByBehalfOf = new Guid("59d7f9ef-c644-41b9-9fc7-e097bc6aa02b")
                        });
                });

            modelBuilder.Entity("SecurityQuestion", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CreatedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ModifiedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ModifiedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<string>("Question")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("Id", "Question")
                        .HasAnnotation("Npgsql:TsVectorConfig", "english");

                    NpgsqlIndexBuilderExtensions.HasMethod(b.HasIndex("Id", "Question"), "GIN");

                    b.ToTable("SecurityQuestions");

                    b.HasData(
                        new
                        {
                            Id = new Guid("700c244d-ebc9-48d9-8073-46b00d69b7d3"),
                            CreatedAt = new DateTime(2023, 3, 29, 9, 24, 26, 910, DateTimeKind.Utc).AddTicks(2275),
                            CreatedBy = new Guid("74bb8c55-292a-4182-b8a5-1ce8c8113891"),
                            CreatedByBehalfOf = new Guid("f501785f-f0cb-421e-9900-bace4368e314"),
                            ModifiedAt = new DateTime(2023, 3, 29, 9, 24, 26, 910, DateTimeKind.Utc).AddTicks(2278),
                            ModifiedBy = new Guid("1586ded0-28c4-4233-94ab-966beb3244a0"),
                            ModifiedByBehalfOf = new Guid("0c5291f0-b326-48d4-ad9c-cfc2d09034da"),
                            Question = "ilk öğretmenin adı"
                        });
                });

            modelBuilder.Entity("User", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CreatedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<string>("EMail")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ModifiedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ModifiedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<string>("Reference")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Salt")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<NpgsqlTsVector>("SearchVector")
                        .IsRequired()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("tsvector")
                        .HasComputedColumnSql("to_tsvector('english', coalesce(\"Reference\", '') || ' ' || coalesce(\"EMail\", '') || ' ' || coalesce(\"FirstName\", '') || ' ' || coalesce(\"LastName\", '') || ' ' || coalesce(\"Number\", ''))", true);

                    b.Property<string>("State")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("Reference")
                        .IsUnique();

                    b.HasIndex("SearchVector");

                    NpgsqlIndexBuilderExtensions.HasMethod(b.HasIndex("SearchVector"), "GIN");

                    b.ToTable("Users");

                    b.HasData(
                        new
                        {
                            Id = new Guid("8a51ec25-b63b-42bc-b913-fd3d7526e072"),
                            CreatedAt = new DateTime(2023, 3, 29, 9, 24, 26, 910, DateTimeKind.Utc).AddTicks(1995),
                            CreatedBy = new Guid("44a19f1d-7446-4107-a2e1-febffb8cb8bb"),
                            CreatedByBehalfOf = new Guid("e6bd35aa-73e2-440f-998a-13e08c1adea2"),
                            EMail = "test@gmail.com",
                            FirstName = "Damla",
                            LastName = "Erhan",
                            ModifiedAt = new DateTime(2023, 3, 29, 9, 24, 26, 910, DateTimeKind.Utc).AddTicks(2002),
                            ModifiedBy = new Guid("b436b274-b4aa-43fb-a80d-9cde141ce17a"),
                            ModifiedByBehalfOf = new Guid("2b65003d-10c7-4a92-a706-022828f05311"),
                            Reference = "12345678912",
                            Salt = "fertrtretregfdgffd",
                            State = "New"
                        });
                });

            modelBuilder.Entity("UserDevice", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("ClientId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CreatedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<int>("DeviceId")
                        .HasColumnType("integer");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ModifiedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ModifiedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<int>("Status")
                        .HasColumnType("integer");

                    b.Property<Guid>("TokenId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.HasIndex("Id", "DeviceId", "UserId")
                        .HasAnnotation("Npgsql:TsVectorConfig", "english");

                    NpgsqlIndexBuilderExtensions.HasMethod(b.HasIndex("Id", "DeviceId", "UserId"), "GIN");

                    b.ToTable("UserDevices");

                    b.HasData(
                        new
                        {
                            Id = new Guid("e8f36277-1cfe-4c63-b360-4555b77449ab"),
                            ClientId = new Guid("d69e5862-48ea-4e24-8675-b76b1c2e79dd"),
                            CreatedAt = new DateTime(2023, 3, 29, 9, 24, 26, 910, DateTimeKind.Utc).AddTicks(2223),
                            CreatedBy = new Guid("1827f97c-df5a-46b3-a356-dc25038a612a"),
                            CreatedByBehalfOf = new Guid("57989328-8b86-4db8-965d-236ba08d7f2f"),
                            DeviceId = 123,
                            ModifiedAt = new DateTime(2023, 3, 29, 9, 24, 26, 910, DateTimeKind.Utc).AddTicks(2224),
                            ModifiedBy = new Guid("87aa3435-31a7-4c4b-b302-6e4c1d014252"),
                            ModifiedByBehalfOf = new Guid("97e805cf-2874-495a-a036-bd529624a48d"),
                            Status = 1,
                            TokenId = new Guid("29c52707-da0e-4bcd-8c80-e678cea8b4e5"),
                            UserId = new Guid("8a51ec25-b63b-42bc-b913-fd3d7526e072")
                        });
                });

            modelBuilder.Entity("UserPassword", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<int?>("AccessFailedCount")
                        .HasColumnType("integer");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CreatedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<string>("HashedPassword")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ModifiedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ModifiedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<bool?>("MustResetPassword")
                        .HasColumnType("boolean");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("UserPasswords");

                    b.HasData(
                        new
                        {
                            Id = new Guid("3b673dd8-c463-4a4d-bfbe-39e7aa172d27"),
                            CreatedAt = new DateTime(2023, 3, 29, 9, 24, 26, 910, DateTimeKind.Utc).AddTicks(2312),
                            CreatedBy = new Guid("ebf9aeae-7c18-4509-8877-3f40393d3227"),
                            CreatedByBehalfOf = new Guid("f74cf7cf-caa6-4051-82c5-b92c77995d7f"),
                            HashedPassword = "",
                            ModifiedAt = new DateTime(2023, 3, 29, 9, 24, 26, 910, DateTimeKind.Utc).AddTicks(2314),
                            ModifiedBy = new Guid("3c4c2958-3047-4e0c-be09-d6869732329a"),
                            ModifiedByBehalfOf = new Guid("1bcb7a74-f183-4c48-aa61-39dd8e7a0f8b"),
                            UserId = new Guid("8a51ec25-b63b-42bc-b913-fd3d7526e072")
                        });
                });

            modelBuilder.Entity("UserSecurityImage", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CreatedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ModifiedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ModifiedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<string>("SecurityImage")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("UserSecurityImages");

                    b.HasData(
                        new
                        {
                            Id = new Guid("7f13a0ad-70ec-4fd8-a167-4fff21fcca19"),
                            CreatedAt = new DateTime(2023, 3, 29, 9, 24, 26, 910, DateTimeKind.Utc).AddTicks(2261),
                            CreatedBy = new Guid("8a4cb4f8-1bf5-4159-872e-3a4d29d5b931"),
                            CreatedByBehalfOf = new Guid("05554731-67c0-4681-ac7a-e7ec72143389"),
                            ModifiedAt = new DateTime(2023, 3, 29, 9, 24, 26, 910, DateTimeKind.Utc).AddTicks(2262),
                            ModifiedBy = new Guid("2c5ad9e6-9b79-4337-9b1d-3abe6e6ce852"),
                            ModifiedByBehalfOf = new Guid("5f603d3b-5202-44d3-bc42-d509a883e195"),
                            SecurityImage = "",
                            UserId = new Guid("8a51ec25-b63b-42bc-b913-fd3d7526e072")
                        });
                });

            modelBuilder.Entity("UserSecurityQuestion", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CreatedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ModifiedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ModifiedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<string>("SecurityAnswer")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("SecurityQuestionId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("SecurityQuestionId")
                        .IsUnique();

                    b.HasIndex("UserId")
                        .IsUnique();

                    b.HasIndex("Id", "UserId", "SecurityQuestionId")
                        .HasAnnotation("Npgsql:TsVectorConfig", "english");

                    NpgsqlIndexBuilderExtensions.HasMethod(b.HasIndex("Id", "UserId", "SecurityQuestionId"), "GIN");

                    b.ToTable("UserSecurityQuestions");

                    b.HasData(
                        new
                        {
                            Id = new Guid("f15efd06-51c5-4950-94fe-365f75551dca"),
                            CreatedAt = new DateTime(2023, 3, 29, 9, 24, 26, 910, DateTimeKind.Utc).AddTicks(2291),
                            CreatedBy = new Guid("adae29ab-f889-4b27-af28-93ffb4fa4cb4"),
                            CreatedByBehalfOf = new Guid("e41e4ff2-dc68-4539-bf12-17bc0a7e97b1"),
                            ModifiedAt = new DateTime(2023, 3, 29, 9, 24, 26, 910, DateTimeKind.Utc).AddTicks(2292),
                            ModifiedBy = new Guid("521864e3-c1f8-4809-88a4-c4dfde971e83"),
                            ModifiedByBehalfOf = new Guid("be452cd3-e144-4d09-8c00-daff8680214e"),
                            SecurityAnswer = "test",
                            SecurityQuestionId = new Guid("700c244d-ebc9-48d9-8073-46b00d69b7d3"),
                            UserId = new Guid("8a51ec25-b63b-42bc-b913-fd3d7526e072")
                        });
                });

            modelBuilder.Entity("UserTag", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CreatedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ModifiedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ModifiedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<string>("Tag")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.HasIndex("Id", "Tag", "UserId")
                        .HasAnnotation("Npgsql:TsVectorConfig", "english");

                    NpgsqlIndexBuilderExtensions.HasMethod(b.HasIndex("Id", "Tag", "UserId"), "GIN");

                    b.ToTable("UserTags");

                    b.HasData(
                        new
                        {
                            Id = new Guid("23a6518a-4926-4dee-8ebb-51e49a24b6e8"),
                            CreatedAt = new DateTime(2023, 3, 29, 9, 24, 26, 910, DateTimeKind.Utc).AddTicks(2206),
                            CreatedBy = new Guid("abda35dc-f03e-4148-a6ed-31052b69ff25"),
                            CreatedByBehalfOf = new Guid("dba310d9-31b0-475e-9ade-78145814941f"),
                            ModifiedAt = new DateTime(2023, 3, 29, 9, 24, 26, 910, DateTimeKind.Utc).AddTicks(2207),
                            ModifiedBy = new Guid("2ec704de-73e8-4757-8a35-d7a251f5db3e"),
                            ModifiedByBehalfOf = new Guid("cf0bcff7-e118-4051-ab49-9963231e178b"),
                            Tag = "user-list-get",
                            UserId = new Guid("8a51ec25-b63b-42bc-b913-fd3d7526e072")
                        });
                });

            modelBuilder.Entity("User", b =>
                {
                    b.OwnsOne("Phone", "Phone", b1 =>
                        {
                            b1.Property<Guid>("UserId")
                                .HasColumnType("uuid");

                            b1.Property<int>("CountryCode")
                                .HasColumnType("integer")
                                .HasColumnName("CountryCode");

                            b1.Property<string>("Number")
                                .IsRequired()
                                .HasColumnType("text")
                                .HasColumnName("Number");

                            b1.Property<int>("Prefix")
                                .HasColumnType("integer")
                                .HasColumnName("Prefix");

                            b1.HasKey("UserId");

                            b1.ToTable("Users");

                            b1.WithOwner()
                                .HasForeignKey("UserId");

                            b1.HasData(
                                new
                                {
                                    UserId = new Guid("8a51ec25-b63b-42bc-b913-fd3d7526e072"),
                                    CountryCode = 90,
                                    Number = "1234564",
                                    Prefix = 530
                                });
                        });

                    b.Navigation("Phone");
                });

            modelBuilder.Entity("UserDevice", b =>
                {
                    b.HasOne("User", "Users")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Users");
                });

            modelBuilder.Entity("UserPassword", b =>
                {
                    b.HasOne("User", "User")
                        .WithMany("UserPasswords")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("UserSecurityImage", b =>
                {
                    b.HasOne("User", "User")
                        .WithMany("UserSecurityImages")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("UserSecurityQuestion", b =>
                {
                    b.HasOne("SecurityQuestion", "SecurityQuestions")
                        .WithMany("UserSecurityQuestion")
                        .HasForeignKey("SecurityQuestionId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("User", "Users")
                        .WithMany("UserSecurityQuestion")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("SecurityQuestions");

                    b.Navigation("Users");
                });

            modelBuilder.Entity("UserTag", b =>
                {
                    b.HasOne("User", "User")
                        .WithMany("UserTags")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("SecurityQuestion", b =>
                {
                    b.Navigation("UserSecurityQuestion");
                });

            modelBuilder.Entity("User", b =>
                {
                    b.Navigation("UserPasswords");

                    b.Navigation("UserSecurityImages");

                    b.Navigation("UserSecurityQuestion");

                    b.Navigation("UserTags");
                });
#pragma warning restore 612, 618
        }
    }
}
