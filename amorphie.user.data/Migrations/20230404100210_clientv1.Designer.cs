// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using NpgsqlTypes;
using amorphie.user.data;

#nullable disable

namespace amorphie.user.data.Migrations
{
    [DbContext(typeof(UserDBContext))]
    [Migration("20230404100210_clientv1")]
    partial class clientv1
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "7.0.0")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("Client", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string[]>("AvailableFlows")
                        .HasColumnType("text[]");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CreatedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("HeaderConfigId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ModifiedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ModifiedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Secret")
                        .HasColumnType("text");

                    b.Property<string>("Status")
                        .HasColumnType("text");

                    b.Property<string[]>("Tags")
                        .HasColumnType("text[]");

                    b.Property<string>("Type")
                        .HasColumnType("text");

                    b.Property<string>("Validations")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("HeaderConfigId");

                    b.ToTable("Clients");
                });

            modelBuilder.Entity("HeaderConfiguration", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("AccessToken")
                        .HasColumnType("text");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CreatedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<string>("DeviceInfo")
                        .HasColumnType("text");

                    b.Property<string>("IdempotencyId")
                        .HasColumnType("text");

                    b.Property<string>("Ip")
                        .HasColumnType("text");

                    b.Property<string>("Jws")
                        .HasColumnType("text");

                    b.Property<string>("Location")
                        .HasColumnType("text");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ModifiedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ModifiedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<string>("SessionId")
                        .HasColumnType("text");

                    b.Property<string>("Variant")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("HeaderConfiguration");
                });

            modelBuilder.Entity("SecurityImage", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CreatedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<string>("Image")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ModifiedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ModifiedByBehalfOf")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.ToTable("SecurityImages");

                    b.HasData(
                        new
                        {
                            Id = new Guid("a93c2b9a-8bf8-4231-b9e3-3e8afe7c53ba"),
                            CreatedAt = new DateTime(2023, 4, 4, 10, 2, 10, 88, DateTimeKind.Utc).AddTicks(3490),
                            CreatedBy = new Guid("20f61393-9715-4282-b8f9-9a7a7bb17387"),
                            CreatedByBehalfOf = new Guid("f6cadfe2-86c9-4df3-be9c-eba8d414b1d7"),
                            Image = "",
                            ModifiedAt = new DateTime(2023, 4, 4, 10, 2, 10, 88, DateTimeKind.Utc).AddTicks(3491),
                            ModifiedBy = new Guid("409b2910-dd24-4c23-9f0f-79284b4bf6b6"),
                            ModifiedByBehalfOf = new Guid("36aa6408-ea0d-4a36-93c4-1af449601999")
                        });
                });

            modelBuilder.Entity("SecurityQuestion", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CreatedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ModifiedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ModifiedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<string>("Question")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("SecurityQuestions");

                    b.HasData(
                        new
                        {
                            Id = new Guid("97d932a7-e96c-46f2-8933-cc1404d03214"),
                            CreatedAt = new DateTime(2023, 4, 4, 10, 2, 10, 88, DateTimeKind.Utc).AddTicks(3531),
                            CreatedBy = new Guid("05cdb5e5-0507-4679-a65d-7450866b9cb8"),
                            CreatedByBehalfOf = new Guid("9ef197cb-437a-4de6-ae91-dff53c77d454"),
                            ModifiedAt = new DateTime(2023, 4, 4, 10, 2, 10, 88, DateTimeKind.Utc).AddTicks(3532),
                            ModifiedBy = new Guid("05997acb-7bed-45cb-aa93-f87d5053268c"),
                            ModifiedByBehalfOf = new Guid("bb650e85-9d21-4b08-a9c7-f67c501b58c4"),
                            Question = "ilk öğretmenin adı"
                        });
                });

            modelBuilder.Entity("User", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CreatedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<string>("EMail")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ModifiedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ModifiedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<string>("Reference")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Salt")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<NpgsqlTsVector>("SearchVector")
                        .IsRequired()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("tsvector")
                        .HasComputedColumnSql("to_tsvector('english', coalesce(\"Reference\", '') || ' ' || coalesce(\"EMail\", '') || ' ' || coalesce(\"FirstName\", '') || ' ' || coalesce(\"LastName\", '') || ' ' || coalesce(\"State\", '') || ' ' || coalesce(\"Number\", ''))", true);

                    b.Property<string>("State")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("Reference")
                        .IsUnique();

                    b.HasIndex("SearchVector");

                    NpgsqlIndexBuilderExtensions.HasMethod(b.HasIndex("SearchVector"), "GIN");

                    b.ToTable("Users");

                    b.HasData(
                        new
                        {
                            Id = new Guid("2e964ac5-7db9-41a5-a5b0-599112f4de60"),
                            CreatedAt = new DateTime(2023, 4, 4, 10, 2, 10, 88, DateTimeKind.Utc).AddTicks(3181),
                            CreatedBy = new Guid("f4586a9e-7341-4327-8b2b-ec0ba732ed58"),
                            CreatedByBehalfOf = new Guid("abf24ca4-52a0-45c9-9bfa-d90cf865099c"),
                            EMail = "test@gmail.com",
                            FirstName = "Damla",
                            LastName = "Erhan",
                            ModifiedAt = new DateTime(2023, 4, 4, 10, 2, 10, 88, DateTimeKind.Utc).AddTicks(3186),
                            ModifiedBy = new Guid("a0f76d6c-bf1e-4520-9d17-7f81b8332805"),
                            ModifiedByBehalfOf = new Guid("dec22d68-b671-4637-a358-973346350490"),
                            Reference = "12345678912",
                            Salt = "fertrtretregfdgffd",
                            State = "New"
                        });
                });

            modelBuilder.Entity("UserDevice", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("ClientId")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CreatedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<int>("DeviceId")
                        .HasColumnType("integer");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ModifiedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ModifiedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<Guid>("TokenId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("UserDevices");

                    b.HasData(
                        new
                        {
                            Id = new Guid("52f5a9cd-464a-4363-8d44-0505ae2267ba"),
                            ClientId = new Guid("a112f0e3-2760-41c9-8a4a-b0f86da0060b"),
                            CreatedAt = new DateTime(2023, 4, 4, 10, 2, 10, 88, DateTimeKind.Utc).AddTicks(3462),
                            CreatedBy = new Guid("0c53456a-e840-4511-9adb-2975e9a1a02a"),
                            CreatedByBehalfOf = new Guid("a347f996-bcf6-4242-a3d8-7026337bfad0"),
                            DeviceId = 123,
                            ModifiedAt = new DateTime(2023, 4, 4, 10, 2, 10, 88, DateTimeKind.Utc).AddTicks(3464),
                            ModifiedBy = new Guid("a6fe2726-d1bd-4d7f-8bae-f3f3d425f95c"),
                            ModifiedByBehalfOf = new Guid("55aa0baa-2ed8-44ee-8e6c-80db93203adf"),
                            TokenId = new Guid("b8e4c70c-9b9f-47aa-8e6b-19a17af8fe20"),
                            UserId = new Guid("2e964ac5-7db9-41a5-a5b0-599112f4de60")
                        });
                });

            modelBuilder.Entity("UserPassword", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<int?>("AccessFailedCount")
                        .HasColumnType("integer");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CreatedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<string>("HashedPassword")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ModifiedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ModifiedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<bool?>("MustResetPassword")
                        .HasColumnType("boolean");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("UserPasswords");

                    b.HasData(
                        new
                        {
                            Id = new Guid("b4a080e6-1f54-44d0-b0d5-96a052e7dc23"),
                            CreatedAt = new DateTime(2023, 4, 4, 10, 2, 10, 88, DateTimeKind.Utc).AddTicks(3574),
                            CreatedBy = new Guid("508b2729-e8ac-4bfa-8fc6-fea1a3e458c7"),
                            CreatedByBehalfOf = new Guid("2aaa95a5-e69e-4131-8fe8-ddd2b699a52a"),
                            HashedPassword = "",
                            ModifiedAt = new DateTime(2023, 4, 4, 10, 2, 10, 88, DateTimeKind.Utc).AddTicks(3578),
                            ModifiedBy = new Guid("228b689e-1290-4582-9e91-c78a2b9e1f8f"),
                            ModifiedByBehalfOf = new Guid("dc22b03e-5e0d-42b9-a322-2eab2370b249"),
                            UserId = new Guid("2e964ac5-7db9-41a5-a5b0-599112f4de60")
                        });
                });

            modelBuilder.Entity("UserSecurityImage", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CreatedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ModifiedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ModifiedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<string>("SecurityImage")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("UserSecurityImages");

                    b.HasData(
                        new
                        {
                            Id = new Guid("831c6adf-8e51-4e93-ad3a-4b3e71ca4715"),
                            CreatedAt = new DateTime(2023, 4, 4, 10, 2, 10, 88, DateTimeKind.Utc).AddTicks(3510),
                            CreatedBy = new Guid("b639dfbc-9cd7-4c9b-92c2-c0578bad8fdf"),
                            CreatedByBehalfOf = new Guid("1008404f-e55e-4b14-a5f2-3cafb7a7851d"),
                            ModifiedAt = new DateTime(2023, 4, 4, 10, 2, 10, 88, DateTimeKind.Utc).AddTicks(3511),
                            ModifiedBy = new Guid("8640e82a-0da4-497e-9d33-2b599a986e77"),
                            ModifiedByBehalfOf = new Guid("f50bb5b3-c4a8-4b74-9eab-7f29b9e68229"),
                            SecurityImage = "",
                            UserId = new Guid("2e964ac5-7db9-41a5-a5b0-599112f4de60")
                        });
                });

            modelBuilder.Entity("UserSecurityQuestion", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CreatedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ModifiedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ModifiedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<string>("SecurityAnswer")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("SecurityQuestionId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("SecurityQuestionId");

                    b.HasIndex("UserId");

                    b.ToTable("UserSecurityQuestions");

                    b.HasData(
                        new
                        {
                            Id = new Guid("0dedcb87-663a-4b0f-8711-4d5da7d9828a"),
                            CreatedAt = new DateTime(2023, 4, 4, 10, 2, 10, 88, DateTimeKind.Utc).AddTicks(3549),
                            CreatedBy = new Guid("7745a5d5-4199-4c53-8d5c-a37dba702a37"),
                            CreatedByBehalfOf = new Guid("551c42d6-ca45-4476-9b18-3e5861232c5c"),
                            ModifiedAt = new DateTime(2023, 4, 4, 10, 2, 10, 88, DateTimeKind.Utc).AddTicks(3550),
                            ModifiedBy = new Guid("1e18c97e-d7c5-4aea-939d-dcf02131c200"),
                            ModifiedByBehalfOf = new Guid("308ad73f-1d33-4e1c-ad4a-ad36e50403b2"),
                            SecurityAnswer = "test",
                            SecurityQuestionId = new Guid("97d932a7-e96c-46f2-8933-cc1404d03214"),
                            UserId = new Guid("2e964ac5-7db9-41a5-a5b0-599112f4de60")
                        });
                });

            modelBuilder.Entity("UserTag", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("CreatedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("CreatedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<DateTime>("ModifiedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<Guid>("ModifiedBy")
                        .HasColumnType("uuid");

                    b.Property<Guid?>("ModifiedByBehalfOf")
                        .HasColumnType("uuid");

                    b.Property<string>("Tag")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("UserTags");

                    b.HasData(
                        new
                        {
                            Id = new Guid("e08a5c60-8723-4eb7-aec4-49bca73770e6"),
                            CreatedAt = new DateTime(2023, 4, 4, 10, 2, 10, 88, DateTimeKind.Utc).AddTicks(3423),
                            CreatedBy = new Guid("544e5860-2e56-40de-8ea1-886d524f96d3"),
                            CreatedByBehalfOf = new Guid("92e836d3-2fb1-412f-9747-a22b526bb6f5"),
                            ModifiedAt = new DateTime(2023, 4, 4, 10, 2, 10, 88, DateTimeKind.Utc).AddTicks(3436),
                            ModifiedBy = new Guid("f5e0ef67-3393-4621-8f31-d1433c466a78"),
                            ModifiedByBehalfOf = new Guid("050a26b3-f892-4fc4-984a-74803cd5a707"),
                            Tag = "user-list-get",
                            UserId = new Guid("2e964ac5-7db9-41a5-a5b0-599112f4de60")
                        });
                });

            modelBuilder.Entity("Client", b =>
                {
                    b.HasOne("HeaderConfiguration", "HeaderConfig")
                        .WithMany()
                        .HasForeignKey("HeaderConfigId");

                    b.Navigation("HeaderConfig");
                });

            modelBuilder.Entity("User", b =>
                {
                    b.OwnsOne("Phone", "Phone", b1 =>
                        {
                            b1.Property<Guid>("UserId")
                                .HasColumnType("uuid");

                            b1.Property<int>("CountryCode")
                                .HasColumnType("integer")
                                .HasColumnName("CountryCode");

                            b1.Property<string>("Number")
                                .IsRequired()
                                .HasColumnType("text")
                                .HasColumnName("Number");

                            b1.Property<int>("Prefix")
                                .HasColumnType("integer")
                                .HasColumnName("Prefix");

                            b1.HasKey("UserId");

                            b1.ToTable("Users");

                            b1.WithOwner()
                                .HasForeignKey("UserId");

                            b1.HasData(
                                new
                                {
                                    UserId = new Guid("2e964ac5-7db9-41a5-a5b0-599112f4de60"),
                                    CountryCode = 90,
                                    Number = "1234564",
                                    Prefix = 530
                                });
                        });

                    b.Navigation("Phone");
                });

            modelBuilder.Entity("UserDevice", b =>
                {
                    b.HasOne("User", "Users")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Users");
                });

            modelBuilder.Entity("UserPassword", b =>
                {
                    b.HasOne("User", "User")
                        .WithMany("UserPasswords")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("UserSecurityImage", b =>
                {
                    b.HasOne("User", "User")
                        .WithMany("UserSecurityImages")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("UserSecurityQuestion", b =>
                {
                    b.HasOne("SecurityQuestion", "SecurityQuestions")
                        .WithMany("UserSecurityQuestion")
                        .HasForeignKey("SecurityQuestionId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("User", "Users")
                        .WithMany("UserSecurityQuestion")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("SecurityQuestions");

                    b.Navigation("Users");
                });

            modelBuilder.Entity("UserTag", b =>
                {
                    b.HasOne("User", "User")
                        .WithMany("UserTags")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("SecurityQuestion", b =>
                {
                    b.Navigation("UserSecurityQuestion");
                });

            modelBuilder.Entity("User", b =>
                {
                    b.Navigation("UserPasswords");

                    b.Navigation("UserSecurityImages");

                    b.Navigation("UserSecurityQuestion");

                    b.Navigation("UserTags");
                });
#pragma warning restore 612, 618
        }
    }
}
